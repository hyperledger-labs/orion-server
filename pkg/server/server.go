...
func New(conf *config.Configurations) (*BCDBHTTPServer, error) {
	...
	var metrics *utils.TxProcessingMetrics
	...
	if conf.LocalConfig.Prometheus.Enabled {
		...
		metrics = utils.NewProcessingMetrics(metricsRegistry)
	...
	db, err := bcdb.NewDB(conf, lg, metrics)
	if err != nil {
	...
	mux.Handle(constants.DataEndpoint, httphandler.NewDataRequestHandler(db, lg, metrics))
