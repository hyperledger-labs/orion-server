	...
	testCases := []struct {
	...
	}{
		{
			...
			requestFactory: func() (*http.Request, error) {
				queryReader := bytes.NewReader(queryBytes)
				require.NotNil(t, queryReader)
				req, err := http.NewRequest(http.MethodPost, constants.URLForDataQuery(dbName), queryReader)
				if err != nil {
					return nil, err
				}
				req.Header.Set(constants.UserHeader, submittingUserName)
		...
		},
		{
			...
			expectedStatusCode: http.StatusForbidden,
			expectedErr:        "error while processing 'POST /data/test_database' because access forbidden",
		},
		{
			...
			expectedStatusCode: http.StatusInternalServerError,
			expectedErr:        "error while processing 'POST /data/test_database' because failed to execute the query",
		},
		{
		...
		},
		{
			name: "empty query",
			requestFactory: func() (*http.Request, error) {
				req, err := http.NewRequest(http.MethodPost, constants.URLForDataQuery(dbName), nil)
				if err != nil {
					return nil, err
				}
				req.Header.Set(constants.UserHeader, submittingUserName)
