...
func (_m *DB) BootstrapDB(conf *config.Configurations) (*types.TxReceiptResponseEnvelope, error) {
	ret := _m.Called(conf)

	var r0 *types.TxReceiptResponseEnvelope
	if rf, ok := ret.Get(0).(func(*config.Configurations) *types.TxReceiptResponseEnvelope); ok {
		r0 = rf(conf)
	} else {
		if ret.Get(0) != nil {
