...
import (
	config "github.com/IBM-Blockchain/bcdb-server/config"
...
func (_m *DB) GetDataProof(userID string, blockNum uint64, dbname string, key string, deleted bool) (*types.ResponseEnvelope, error) {
	ret := _m.Called(userID, blockNum, dbname, key, deleted)

	var r0 *types.ResponseEnvelope
	if rf, ok := ret.Get(0).(func(string, uint64, string, string, bool) *types.ResponseEnvelope); ok {
