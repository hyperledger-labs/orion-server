...
func (c *Cluster) AgreedHeight(t *testing.T, expectedBlockHeight uint64, activeServers ...int) bool {
	for _, srvVal := range activeServers {
		...
		if blockResEnv != nil {
			t.Logf("block number = %v", blockResEnv.GetResponse().GetBlockHeader().GetBaseHeader().GetNumber())
		}
		if err != nil || blockResEnv == nil || blockResEnv.GetResponse().GetBlockHeader().GetBaseHeader().GetNumber() != expectedBlockHeight {
			return false
		}
	}
	return true
