...
func TestDataTx(t *testing.T) {
	for _, enableTls := range []bool{false, true} {
		t.Run(fmt.Sprintf("TLS:%v", enableTls), func(t *testing.T) {
			...
			t.Run("metrics", func(t *testing.T) {
				client := http.Client{Transport: &http.Transport{TLSClientConfig: s.NewTLSConfig(t)}}

				resp, err := client.Get(s.PrometheusURL(enableTls))
				require.NoError(t, err)
